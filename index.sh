...
if ! [ -x "$(command -v curl)" ]; then
    ...
    LATEST_VER=$(curl -s https://raw.githubusercontent.com/andr3wV/MuteAdsSpotify/master/NoAdsSpotify.sh | grep "CURRENT_VER=" | head -n1 | awk -F '=' '{print $2}')
    
    if [ $CURRENT_VER -ne $LATEST_VER ]; then
        read -n 1 -p "A new version is available, will you like to update? [y/N]" update
        if [ -n "$update" ]; then
            if [ $update == "y" ]; then
                curl https://raw.githubusercontent.com/andr3wV/MuteAdsSpotify/master/NoAdsSpotify.sh > $INSTALLATION_DIR/NoAdsSpotify.sh
                echo "Update finish. 🥳"
...
echo "This program was downloaded from https://andr3wV.github.io/MuteAdsSpotify/ (check for documentation)" 
echo
echo "If the program is not working properly please open an issue at: https://github.com/andr3wV/MuteAdsSpotify/issues"
echo
...
trap 'echo ">>> ⛔️ 👎 Oops! Something failed and the program is not running. Please open a new issue at: https://github.com/andr3wV/NoAdsSpotify/issues/new and copy and paste the whole output of this window into so I can fix it."' EXIT
